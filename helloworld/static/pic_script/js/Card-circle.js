...
var Card = (function (window, undefined) {
    ...
    function Card(id, el) {
        ...
        this._content = $(this.parent).find(SELECTORS.content)[0];
    ...
    Card.prototype.openCard = function (callback) {
        ...
        const full_title = $(".wrapper");
        $.each(full_title, function (val, index) {
            $(this).find(".full_title").css("display", "block");
        });

        try {
            IFrameResize(document.getElementsByClassName("card__content")[0].clientHeight - 650);
        } catch (e) {
    ...
    Card.prototype.closeCard = function () {
        ...
        const full_title = $(".wrapper");
        $.each(full_title, function (val, index) {
            $(this).find(".full_title").css("display", "none");
            $(this).find(".full_title").eq(0).css("display", "block");
        });
