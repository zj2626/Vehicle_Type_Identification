var classification = function (object, rd, vehicle, detect_time) {
    let info = $("#info");
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            ...
            if (data.message == 0) {
                $("#table_show").find("table").remove();

                let predict = data.predict;
                let l_name = "";
                let l_pred = "";
                $.each(predict, function (index, val) {
                    l_name += val[0][0].label + "&nbsp;&nbsp;&nbsp;";
                    l_pred += val[0][0].prob + "%&nbsp;&nbsp;&nbsp;";

                    try {
                        let table = '<table border="4" class="table" id="table' + index + '">\n' +
                            '                            <tr class="t_tr">\n' +
                            '                                <th class="t_th">类型</th>\n' +
                            '                                <th class="t_th">概率</th>\n' +
                            '                            </tr>\n' +
                            '                        </table>';
                        $("#table_show").append(table);

                        $.each(val[0], function (index1, val1) {
                            let tr = '<tr class="t_tr">\n' +
                                '       <td class="t_td">' + val1.label + '</td>\n' +
                                '       <td class="t_td">' + val1.prob + '%</td>\n' +
                                '     </tr>';
                ...
                vehicle.html(l_name);
                let ios = "检测时间:" + detect_time + "秒 | 识别时间:" + data.time + "秒 " +
                    "| 类别概率：" + l_pred;
                info.html(ios);

                $("#" + rd).attr("label", l_name);
                $("#" + rd).attr("time", ios);
            } else {
                vehicle.text("未能识别");
                info.html("");
    ...
    $.fn.zyUpload = function (options, param) {
        ...
        return this.each(function () {
            ...
            var defaults = {
                ...
                },// 选择图片的回调方法  selectFile:当前选中的图片  allFiles:还没上传的全部图片
                onDelete: function (file, files) {
                },     // 删除一个图片的回调方法 file:当前删除的图片  files:删除之后的图片
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '				<div class="btns">';
                    html += '					<div class="upload_btn">上传识别</div>';
                    html += '				</div>';
                    html += '			</div>';
                    html += '			<div id="preview" hidden class="upload_preview" style="margin:1px 0 1px 0"></div>';
                    html += '		</div>';
                    ...
                    html += '				</div>';
                    html += '			</div>';
                    html += '			<div id="preview" hidden class="upload_preview" style="margin:1px 0 1px 0">';
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                if (file.type.indexOf("image") == 0) {
                    ...
                    html += '			<img id="uploadImage_' + file.index + '" ' +
                        'onclick="changePic(' + file.index + ', this)" ' +
                        'class="upload_image" ' +
                        'src="' + result + '" ' +
            ...
            this.createCorePlug = function () {
                var params = {
                    ...
                    onDelete: function (file, files) {
                        ...
                        $("#back-img").attr('src', $("#back-img").attr("default-src"));
                        $("#vehicle").text($("#vehicle").attr("default-name"));
                        $("#info").text("");
                    ...
                    onSuccess: function (file, response) {
                        ...
                        const rd = 'i' + zimu(5);
                        $("#uploadInf").prepend('<p id="' + rd + '" class="i_message" label="" time="">上传成功, 图片地址是：<br/>' + object.address);
                        ...
                        $.ajax({
                            ...
                            success: function (data, textStatus, jqXHR) {
                                console.log(textStatus);
                                if (data.message == 0) {
                                    $("#back-img").attr('src', data.full_address);
                                    detect_time = data.time;
                                } else {

                                }

                                classification(object, rd, vehicle, detect_time);
                            },
                            error: function (xhr, textStatus) {
                                vehicle.text("未能识别");
                                //console.log(textStatus);
                    ...
                    onFailure: function (file) {
                        $("#uploadProgress_" + file.index).hide();
                        $("#uploadSuccess_" + file.index).show();
                        $("#uploadInf").text("<p>图片" + file.name + "上传失败！</p>");
            ...
            this.addEvent = function () {
                ...
                $(".upload_btn").bind("click", function (e) {
                    ...
                    if (ZYFILE.funReturnNeedFiles().length > 0) {
                        $("#fileSubmit").click();
                        $("#vehicle").html(load);
                        $("#vehicle_load").html('<img src="static/control/images/load.gif" />');
                    } else {
                        alert("请先选中图片再点击上传");
                    }
                });
